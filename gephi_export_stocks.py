    ...
    for j in range(iterations):
        for i, text in enumerate(data):
            ...
            if len(title_column) == 0:
                ...
                mapping[i + len(data)*j] = df[title_column[dataset_iteration]][i + len(data)*j]
...
def run_all(datasets, models, n, graph=False, batch_size=16, iterations=1, content_column=[], title_column=[]):
    model_dict = {}
