...
class SparseTransformer(nn.Module):
    ...
    def train_and_validate(self, X_train, X_val, learning_rate=5e-5, batch_size=4096, 
            ...
            # Configure data loaders with pin_memory for faster transfer to GPU
            ...
            # Set up progress tracking with tqdm - matching SAE style
            progress_bar = tqdm(
            ...
            progress_bar.set_postfix({
                'd%': f"{0:.0f}",
                'L': f"{self.lambda_l1:.1f}",  # Using 'L' instead of lambda character for Windows compatibility
            ...
            for epoch in range(num_epochs):
                ...
                for batch_idx, batch in enumerate(train_loader):
                    ...
                    if self.use_mixed_precision:
                        ...
                        running_loss += scaled_loss.item() * grad_accum_steps  # Track the actual unscaled loss
                    else:
                    ...
                    if step % eval_freq == 0:
                        ...
                        # Update training history (only once)
            ...
            # Print a summary like in SAE.py
            summary = (
                f"\n{'='*60}\n"
                ...
                f"Final L1: {self.lambda_l1:.2f}"  # Changed from λ to L1 for Windows compatibility
            )
            self.logger.info(summary)
            
            # Final model save - just the model with no checkpoints
            checkpoint = {
                ...
                'feature_dim': self.m,
                'attention_dim': self.a,
                'attention_fn': self.attention_fn_name,
                'steps': step,
